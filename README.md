...
- 图片、文档、音频、视频分类导航
- 激活码保护的文档工具：PDF 转 DOCX/XLSX/PPTX、扫描件中英文 OCR、Word 规范排版、简历美化、PPT 文字版式重建、PDF 合并拆分压缩
- 独立音频下载页：公开媒体直链解析、授权视频提取音频、六种音频格式输出
- 响应式中文界面，支持拖放或选择文件

PDF 转 Office 和 Word/PPT 重新排版采用浏览器端提取与重建，不承诺复杂表格、公式、图片、动画和特殊字体无损保留；每个工具会在执行前显示对应限制。
...
登录管理员账号，进入 `/admin/`，点击生成一个 5 次转换激活码。明文激活码只显示一次；数据库只保存 SHA-256 哈希和遮罩提示。立即复制，并通过闲鱼聊天发送给对应买家。每个激活码只能绑定一个账号。
