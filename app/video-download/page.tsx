...
type ParseResult = {
  platform: "direct"; title: string; coverUrl: string | null; durationSeconds: number | null;
...
const platforms = [
  ...
  { name: "抖音公开视频", detail: "等待合规接口", ready: false },
...
export default function VideoDownloadPage() {
  ...
  const parse = async () => {
    ...
    try {
      ...
      setResult(parsed); setQuality(parsed.qualities[0]?.id || "original");
  ...
  return <main className="min-h-screen overflow-hidden bg-[#fafafe] text-slate-900">
    ...
    <section className="relative z-10 mx-auto max-w-6xl px-5 pb-16 pt-14 sm:px-8 sm:pt-20">
      ...
      <div className="mx-auto mt-10 max-w-4xl rounded-[28px] border border-white/80 bg-white/90 p-5 shadow-[0_24px_70px_rgba(79,70,229,0.12)] backdrop-blur sm:p-8">
        {!auth.configured && <div className="mb-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">站点尚未配置 Supabase，登录、解析和下载暂不可用。</div>}
        ...
        {result && <section className="mt-6 overflow-hidden rounded-2xl border border-slate-200"><div className="flex flex-col gap-4 bg-gradient-to-br from-slate-950 to-violet-950 p-5 text-white sm:flex-row sm:items-center"><div className="grid aspect-video w-full place-items-center rounded-xl bg-white/10 sm:w-44"><FileVideo2 className="h-10 w-10 text-violet-200" /></div><div className="min-w-0 flex-1"><span className="rounded-full bg-emerald-400/15 px-2.5 py-1 text-xs text-emerald-200">普通视频直链</span><h2 className="mt-3 truncate text-lg font-semibold">{result.title}</h2><p className="mt-2 text-xs text-white/60">{formatBytes(result.contentLength)} · {result.contentType} · 时长由源站提供时显示</p></div></div><div className="p-5"><label className="text-xs font-semibold text-slate-500">选择清晰度<select value={quality} onChange={(event) => setQuality(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-slate-200 px-3 text-sm text-slate-800">{result.qualities.map((item) => <option key={item.id} value={item.id}>{item.label} · {formatBytes(item.size)}</option>)}</select></label><button onClick={() => void downloadVideo()} disabled={Boolean(busy)} className="mt-4 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-violet-600 text-sm font-semibold text-white disabled:opacity-50">{busy === "download" ? <><LoaderCircle className="h-4 w-4 animate-spin" />下载中 {progress}%</> : <><Download className="h-4 w-4" />下载视频</>}</button><div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500"><span>今日解析 {result.usage.usedToday}/{result.usage.dailyLimit}</span><Link href="/" className="inline-flex items-center gap-1 font-semibold text-violet-700">下载后继续转换为 MP4、MP3 <ExternalLink className="h-3 w-3" /></Link></div></div></section>}
      </div>
