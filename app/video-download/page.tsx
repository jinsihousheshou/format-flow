...
type ParseResult = {
  platform: "direct" | "douyin"; title: string; coverUrl: string | null; durationSeconds: number | null;
  contentType: string; contentLength: number;
  qualities: { id: string; label: string; contentType: string; size: number }[];
...
const platforms = [
  ...
  { name: "抖音公开视频", detail: "官方播放器预览", ready: true },
  { name: "快手公开视频", detail: "等待合规接口", ready: false },
  { name: "哔哩哔哩公开视频", detail: "等待合规接口", ready: false },
];
...
export default function VideoDownloadPage() {
  ...
  const parse = async () => {
    ...
    setBusy("parse");
    try {
      const parsed = await edgeFunction<ParseResult>("video-link", { action: "parse", input, rightsConfirmed: true }, auth.session!.access_token);
  ...
  const downloadVideo = async () => {
    ...
    try {
      const response = await fetch(`${supabaseUrl}/functions/v1/video-link`, {
      ...
      const total = Number(response.headers.get("content-length")) || result.contentLength;
      const reader = response.body.getReader(); const chunks: ArrayBuffer[] = []; let received = 0;
      while (true) { const { done, value } = await reader.read(); if (done) break; if (value) { const copy = new Uint8Array(value.byteLength); copy.set(value); chunks.push(copy.buffer); received += value.byteLength; setProgress(Math.min(100, Math.round(received / total * 100))); } }
      const blob = new Blob(chunks, { type: response.headers.get("content-type") || result.contentType });
  ...
  return <main className="min-h-screen overflow-hidden bg-[#fafafe] text-slate-900">
    ...
    <section className="relative z-10 mx-auto max-w-6xl px-5 pb-16 pt-14 sm:px-8 sm:pt-20">
      ...
      <div className="mx-auto mt-10 max-w-4xl rounded-[28px] border border-white/80 bg-white/90 p-5 shadow-[0_24px_70px_rgba(79,70,229,0.12)] backdrop-blur sm:p-8">
        ...
        {result && <section className="mt-6 overflow-hidden rounded-2xl border border-slate-200">{result.embedUrl ? <div className="bg-slate-950 p-3 sm:p-5"><iframe src={result.embedUrl} title={result.title} className="mx-auto aspect-[9/16] max-h-[560px] w-full max-w-[315px] rounded-xl bg-black" allow="autoplay; fullscreen" allowFullScreen referrerPolicy="strict-origin-when-cross-origin" /></div> : <div className="flex flex-col gap-4 bg-gradient-to-br from-slate-950 to-violet-950 p-5 text-white sm:flex-row sm:items-center"><div className="grid aspect-video w-full place-items-center rounded-xl bg-white/10 sm:w-44"><FileVideo2 className="h-10 w-10 text-violet-200" /></div><div className="min-w-0 flex-1"><span className="rounded-full bg-emerald-400/15 px-2.5 py-1 text-xs text-emerald-200">普通视频直链</span><h2 className="mt-3 truncate text-lg font-semibold">{result.title}</h2><p className="mt-2 text-xs text-white/60">{formatBytes(result.contentLength)} · {result.contentType} · 时长由源站提供时显示</p></div></div>}<div className="p-5"><div className="flex items-start justify-between gap-3"><div><span className="text-xs font-semibold text-violet-600">{result.platform === "douyin" ? "抖音官方公开播放器" : "解析完成"}</span><h2 className="mt-1 text-base font-semibold text-slate-900">{result.title}</h2>{result.width && result.height && <p className="mt-1 text-xs text-slate-500">画面尺寸 {result.width} × {result.height}</p>}</div></div>{result.notice && <p className="mt-4 rounded-xl bg-amber-50 p-3 text-sm leading-6 text-amber-800">{result.notice}</p>}{result.qualities.length > 0 && <label className="mt-4 block text-xs font-semibold text-slate-500">选择清晰度<select value={quality} onChange={(event) => setQuality(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-slate-200 px-3 text-sm text-slate-800">{result.qualities.map((item) => <option key={item.id} value={item.id}>{item.label} · {formatBytes(item.size)}</option>)}</select></label>}<button onClick={() => void downloadVideo()} disabled={Boolean(busy) || result.downloadAvailable === false} className="mt-4 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-violet-600 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:bg-slate-300">{result.downloadAvailable === false ? "平台未开放直接下载" : busy === "download" ? <><LoaderCircle className="h-4 w-4 animate-spin" />下载中 {progress}%</> : <><Download className="h-4 w-4" />下载视频</>}</button><div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500"><span>今日解析 {result.usage.usedToday}/{result.usage.dailyLimit}</span>{result.downloadAvailable !== false && <Link href="/" className="inline-flex items-center gap-1 font-semibold text-violet-700">下载后继续转换为 MP4、MP3 <ExternalLink className="h-3 w-3" /></Link>}</div></div></section>}
      </div>
