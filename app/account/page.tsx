...
const planNames = { trial: "体验码", monthly: "月卡", yearly: "年卡", lifetime: "长期版", credits: "次数卡" };
  ...
  return <main className="min-h-screen bg-[#fbfbfe]"><SiteHeader /><section className="mx-auto max-w-6xl px-5 py-10 sm:px-8 sm:py-14">
    ...
    {!auth.session ? <div className="mt-8 rounded-[28px] border border-violet-100 bg-white p-8 text-center shadow-sm"><ShieldCheck className="mx-auto h-10 w-10 text-violet-600" /><h2 className="mt-4 text-xl font-bold">请先登录账号</h2><p className="mt-2 text-sm text-slate-500">登录后可以兑换激活码并查看使用权益。</p><button onClick={() => setAuthOpen(true)} className="mt-5 rounded-xl bg-violet-600 px-5 py-3 text-sm font-semibold text-white">登录 / 注册</button></div> : <>
      <div className="mt-8 grid gap-5 md:grid-cols-3">
        <article className="rounded-[24px] border border-violet-100 bg-gradient-to-br from-violet-600 to-indigo-700 p-6 text-white shadow-lg shadow-violet-500/15"><span className="text-sm text-white/70">当前套餐</span><h2 className="mt-3 text-2xl font-bold">{entitlement ? planNames[entitlement.plan_type] : "尚未激活"}</h2><span className={`mt-5 inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold ${active ? "bg-white/15" : "bg-rose-400/25"}`}>{active ? <CheckCircle2 className="h-4 w-4" /> : <Clock3 className="h-4 w-4" />}{active ? "权限有效" : "需要激活"}</span></article>
        ...
        <article className="rounded-[24px] border border-slate-200 bg-white p-6"><RefreshCw className="h-6 w-6 text-violet-600" /><p className="mt-4 text-sm text-slate-500">剩余转换次数</p><p className="mt-2 text-3xl font-bold text-slate-900">{entitlement?.remaining_conversions === null ? "不限" : entitlement?.remaining_conversions ?? 0}</p><button onClick={() => setAuthOpen(true)} className="mt-3 inline-flex items-center gap-1.5 text-sm font-semibold text-violet-700"><KeyRound className="h-4 w-4" />兑换新卡</button></article>
      </div>
