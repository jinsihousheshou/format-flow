...
const planNames: Record<string, string> = { trial: "体验码", monthly: "月卡", yearly: "年卡", lifetime: "长期版", credits: "次数卡" };
...
export default function AdminPage() {
  ...
  const updateVideoLimit = async (limit: PlanVideoLimit) => {
    ...
    const maxMb = prompt("单个视频最大 MB（1–100）？", String(Math.round(limit.max_video_bytes / 1048576))); if (maxMb === null) return;
    setBusy(true); setMessage("");
    try { await callAdmin("update-video-limits", { planType: limit.plan_type, dailyParseLimit: Number(parses), dailyDownloadLimit: Number(downloads), maxVideoMb: Number(maxMb) }); setMessage("视频链接限额已更新。"); await load(); }
  ...
  return <main className="min-h-screen bg-[#f7f7fc]"><SiteHeader /><section className="mx-auto max-w-7xl px-5 py-10 sm:px-8">
    ...
    <section className="mt-6 rounded-[24px] border border-slate-200 bg-white p-6"><div className="flex items-center gap-3"><Gauge className="h-5 w-5 text-violet-600" /><h2 className="text-lg font-bold">视频链接套餐限额</h2></div><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{dashboard.planLimits.map((limit) => <article key={limit.plan_type} className="rounded-2xl border border-slate-200 p-4"><p className="font-semibold text-slate-900">{planNames[limit.plan_type] || limit.plan_type}</p><p className="mt-2 text-xs leading-6 text-slate-500">解析 {limit.daily_parse_limit} 次/天<br />下载 {limit.daily_download_limit} 次/天<br />单文件 {Math.round(limit.max_video_bytes / 1048576)} MB</p><button disabled={busy} onClick={() => void updateVideoLimit(limit)} className="mt-3 text-xs font-semibold text-violet-700 disabled:opacity-50">配置限额</button></article>)}</div></section>
